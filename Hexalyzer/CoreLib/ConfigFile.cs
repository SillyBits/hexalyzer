...
{
	//
	// Settings handling using XML approach
	...
	//
	//     Window.SetPosition(
	//     Settings.Root.mru.files.Add(<newly opened file>)
	//
...
public class Config : DynamicObject
{
	...
	private static Hexalyzer.ConfigFile C
	{
		get
		{
			if (Hexalyzer.ConfigFile.CONFIG == null)
				throw new ArgumentNullException("No config available");
			return Hexalyzer.ConfigFile.CONFIG;
		}
	}
